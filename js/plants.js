...
var CPlants = NewO({
		...
		*/
		...
		CanGrow(data, R, C) {
			...
			if ((oGd.$GdType[R][C] === 1 || self.FlyingPlant || oGd.$GdType[R][C] === 2 && oGd.$WaterDepth[R][C] ===
					0) && oGd.$GdType[R][C] !== 0) {
				//如果是草地地形或无视地形的飞行植物，荒地强制禁止种植
				return !(C < 1 || C > 9 || oGd.$Crater[flatCoord] || oGd.$LockingGrid[flatCoord] && self.PKind !=
					5 || data[self.PKind]) && (!data[1] || data[1]
					.canShovel
				); //     在可种植列以内        非坑洞                       非锁定格子且不是瓷砖                         没有当前种类植物    其他植物要种植时需保证没有普通植物或者普通植物可铲除
			} else {
		...
		Birth(X, Y, R, C, plantsArg) {
			...
			self.InitTrigger(self, id, self.R = R, self.C = C, self.AttackedLX = pixelLeft + self
				.beAttackedPointL, //植物左检测点
		...
		BoomGIF(left, top) {
			...
			oEffects.ImgSpriter({
				ele: NewEle(self.id + '_Boom', "div",
					`position:absolute;overflow:hidden;z-index:${self.zIndex + 2};width:216px;height:164px;left:${left}px;top:${top}px;background:url(images/Plants/CherryBomb/Boom.png) no-repeat;`,
					0, EDPZ),
				styleProperty: 'X',
				changeValue: -216,
				frameNum: 25
	...
	oLawnCleaner = InheritO(CPlants, {
		...
		beAttackedPointL: 0,
		beAttackedPointR: 60,
		PicArr: ["images/interface/LawnCleaner.png", "images/Card/LawnCleaner.webp",
			'images/Plants/SporeShroom/Effect.webp'
		],
		AudioArr: ["lawnmower"],
		NormalGif: 0,
		CardGif: 1,
		...
		PlayBirthEffect(obj) {
			const ele = NewEle(obj.id + "Effect", "div",
				`position:absolute;z-index:${obj.zIndex - 1};width:631px;height:481px;left:${obj.pixelLeft - 270}px;top:${obj.pixelTop - 260}px;transform:scale(0.5,0.5);background:url(${obj.PicArr[2]}) no-repeat;`,
				0, EDPZ);
		...
		TriggerCheck: function(b, a) {
			b.beAttacked && b.Altitude > 0 && (this.canTrigger = 0, this.NormalAttack(this), localStorage
				.JNG_DEV !== 'true' && CSpeed(1));
		},
		NormalAttack: function(a) {
			...
			(function fun(b, c, k, j, e, g) {
				...
				if (k > c) {
					...
					oSym.addTask(10, _ => {
						let ele = NewImg(0, src,
							`width:160px;height:195px;top:${GetY(b.R) - 170}px;left:${b.AttackedRX - 50}px;`,
							EDPZ);
						oSym.addTask(113, _ => {
							ClearChild(ele);
						});
	...
	oPeashooter = InheritO(CPlants, {
		...
		PicArr: function() {
			...
			var b = "images/Plants/";
			return ["images/Card/Peashooter.webp", a + "0.webp", a + "Peashooter.webp", b + "PB00.webp", b +
				"PeaBulletHit0.webp", a + "PeashooterAttack.webp"
			];
		}(),
		...
		NormalAttack() {
			...
			oSym.addTask(60, _ => {
				...
				CustomBullet(oPeaBullet, [self.PicArr[self.BulletGif]], self.AttackedLX, self.pixelTop + 15,
					self.R);
			});
		}
			...
			if (oS.ProduceSun) {
				...
				oSym.addTask(500, function fun(X, Y) {
					...
					$P[id] && ( //植物还存在
						...
						oSym.addTask(75, _ => {
							if ($P[id]) {
								AppearSun(X - 30, Y - 50, 50, 0, 50 - Math.random() *
									10); //生产阳光

								oSym.addTask(100, _ => $P[id] && self.ChangePosition(ele,
									0)); //切回动画
	...
	oPotatoMine = InheritO(CPlants, {
		...
		PicArr: function() {
			...
			return ["images/Card/PotatoMine.webp", a + "0.gif", a + "PotatoMineNotReady.png", a +
				"PotatoMine.gif", a + "Boom.png", a + "PotatoMineOut.gif"
			];
		}(),
		...
		TriggerCheck(zombie) {
			...
			*/
			...
			zombie.beAttacked && zombie.Altitude < 2 && !zombie.isFloating && !oGd.$[self.R + "_" + self.C +
				"_2"] && self.NormalAttack(self.pixelLeft, self.pixelRight, self.R);
		},
		...
		NormalAttack: function(lx, rx, r) {
			...
			oEffects.ImgSpriter({
				ele: NewEle(id + '_Boom', "div",
					`position:absolute;overflow:hidden;z-index:${self.zIndex + 2};width:213px;height:210px;left:${self.pixelLeft - 50}px;top:${self.pixelTop - 10}px;background:url(images/Plants/PotatoMine/Boom.png) no-repeat;`,
					0, EDPZ),
				styleProperty: 'X',
				changeValue: -213,
				frameNum: 15,
	...
	oWallNut = InheritO(CPlants, {
		...
		PicArr: function() {
			...
			return ["images/Card/WallNut.webp", a + "0.gif", a + "WallNut.webp", a + "Wallnut_cracked1.webp",
				a + "Wallnut_cracked2.webp"
			];
		}(),
		...
		CanGrow: function(c, b, f) {
			var a = b + "_" + f,
				d = c[1],
				e = oS.ArP;
			return e ? oGd.$GdType[b][f] == 1 ? f > 0 && f < e.ArC[1] && !(oGd.$Crater[a] || oGd.$LockingGrid[
				a] || d) : c[0] && !d : d && d.HP <= 2667 && d.EName == "oWallNut" ? 1 : oGd.$GdType[b][
				f
			] == 1 || oGd.$GdType[b][f] === 2 && oGd.$WaterDepth[b][f] === 0 ? !(f < 1 || f > 9 || oGd
				.$Crater[a] || oGd.$LockingGrid[a] || d) : c[0] && !d;
		},
		...
		getHurt: function(e, b, a) {
			...
			!(b % 3) ? (c.HP -= a) < 1 ? c.Die() : c.HP < 1334 ? c.HurtStatus < 2 && (c.HurtStatus = 2, d.src =
				"images/Plants/WallNut/Wallnut_cracked2.webp") : c.HP < 2667 && c.HurtStatus < 1 && (c
				.HurtStatus = 1, d.src = "images/Plants/WallNut/Wallnut_cracked1.webp"): c.Die(1);
		}
	}),
	//森林植物从以下开始
	oStoneFlower = InheritO(CPlants, {
		...
		NormalAttack: function(zid) {
			...
			let zombie = $Z[zid],
				o = this,
				pid = o.id;
			!o.isAttacking && ($(pid).childNodes[1].src = o.PicArr[3], o.isAttacking = 1, oSym.addTask(50,
				function fun() {
					if ($P[pid]) {
						o.ArZ.length < 1 ? ($(pid).childNodes[1].src = o.PicArr[2], o.isAttacking = 0) :
							oSym.addTask(50, fun);
					}
				}));
		...
		TriggerCheck: function(i, h) {
			var c = i.id,
				...
				e,
				f;
			i.PZ && !g[c] && (a = i.AttackedLX, b = i.AttackedRX, e = this.AttackedLX, f = this.AttackedRX, a <=
				f && a >= e || b <= f && b >= e || a <= e && b >= f) && this.AttackCheck2(i) && (g[c] =
				1, //把当前僵尸标注为已检查过
	...
	oRadish = InheritO(CPlants, {
		...
		PicArr: function() {
			...
			return ["images/Card/Radish.webp", a + "0.webp", a + "Radish.webp", a + "Attack.webp", a +
				"Bullet.png"
			];
		}(),
		...
		NormalAttack() {
			...
			oSym.addTask(62.5, _ => {
				PlayAudio("Radish");
				CustomBullet(oRadishBullet, [self.PicArr[self.BulletGif]], self.AttackedLX, self.pixelTop +
					30, self.R);
			});
		}
	...
	oSnowPea = InheritO(oPeashooter, {
		...
		PicArr: function() {
			...
			var b = "images/Plants/";
			return ["images/Card/SnowPea.webp", a + "0.webp", a + "SnowPea.webp", b + "PB-10.webp", b +
				"PeaBulletHit1.webp", a + "PeashooterAttack.webp"
			];
		}(),
		AudioArr: ["frozen", "splat1", "splat2", "splat3", "shieldhit", "shieldhit2", "plastichit", 'throw',
			'throw2'
		],
		...
		NormalAttack: function() {
			...
			oSym.addTask(60, function() {
				PlayAudio(['throw', 'throw2'][Math.round(Math.random() * 1)]);
				CustomBullet(oSnowPeaBullet, [self.PicArr[self.BulletGif]], self.AttackedLX, self
					.pixelTop + 15, self.R);
			});
		}
	}),
	oRepeater = InheritO(oPeashooter, {
		...
		PicArr: function() {
			...
			var b = "images/Plants/";
			return ["images/Card/Repeater.webp", a + "0.webp", a + "Repeater.webp", b + "PB00.webp", b +
				"PeaBulletHit0.webp", a + "RepeaterAttack.webp"
			];
		}(),
		...
		NormalAttack: function(a) {
			...
			oSym.addTask(75, function(c) {
				let b = $P[c];
				b && CustomBullet(oPeaBullet, [b.PicArr[b.BulletGif]], b.AttackedLX, b.pixelTop + 15, b
					.R);
			}, [this.id]);
		}
	}),
	...
	oTallNut = InheritO(oWallNut, {
		...
		PicArr: function() {
			...
			return ["images/Card/TallNut.webp", a + "0.gif", a + "TallNut.gif", a + "TallnutCracked1.gif", a +
				"TallnutCracked2.gif"
			];
		}(),
		...
		CanGrow: function(c, b, f) {
			var a = b + "_" + f,
				...
				e = oS.ArP;
			return e ? oGd.$GdType[b][f] == 1 ? f > 0 && f < e.ArC[1] && !(oGd.$Crater[a] || oGd.$LockingGrid[
				a] || d) : c[0] && !d : d && d.HP <= 8000 && d.EName == "oTallNut" ? 1 : oGd.$GdType[b][
				f
			] == 1 || oGd.$GdType[b][f] === 2 && oGd.$WaterDepth[b][f] === 0 ? !(f < 1 || f > 9 || oGd
				.$Crater[a] || oGd.$LockingGrid[a] || d) : c[0] && !d;
		},
		...
		getHurt: function(e, b, a) {
			...
			!(b % 3) ? (c.HP -= a) < 1 ? c.Die() : c.HP < 4000 ? c.HurtStatus < 2 && (c.HurtStatus = 2, d.src =
				"images/Plants/TallNut/TallnutCracked2.gif") : c.HP < 8000 && c.HurtStatus < 1 && (c
				.HurtStatus = 1, d.src = "images/Plants/TallNut/TallnutCracked1.gif"): c.Die(1);
		}
	}),
	//沼泽植物从以下开始
	oSunShroom = InheritO(CPlants, {
		...
		PicArr: function() {
			...
			return ["images/Card/SunShroom.webp", a + "0.webp", a + "SunShroom2.webp", a + "SunShroom.webp", a +
				"SunShroomGrow.webp", a + "ProduceSun.webp"
			];
		}(),
		...
		ChangePosition: function(id, a) {
			if ($P[id]) {
				...
				dom.childNodes && (a ? dom.childNodes[1].src = obj.PicArr[5] : dom.childNodes[1].src = obj
					.PicArr[3]);
			}
		}
	}),
	oPuffShroom = InheritO(CPlants, {
		...
		PicArr: function() {
			...
			var b = "images/Plants/";
			return ["images/Card/PuffShroom.webp", a + "0.gif", a + "PuffShroom.gif", a +
				"PuffShroomAttack.gif", b + "ShroomBullet.png", b + "ShroomBulletHit.gif"
			];
		}(),
		AudioArr: ["puff"],
		...
		PrivateBirth: function(a) {
			a.BulletEle = NewImg(0, "images/Plants/ShroomBullet.png", "left:" + (a.AttackedLX - 46) +
				"px;top:" + (a.pixelTop + 35) + "px;visibility:hidden;z-index:" + (a.zIndex + 2));
		},
		PrivateDie: function(a) {
			a.BulletEle = null;
		},
			...
			oSym.addTask(1, function fun(j, d, e, f, g, v) {
				...
				if (v[f + "_" + i]) {
					...
					h && h.Altitude == 1 ? (h.getPea(h, b.Attack, 0), SetStyle(d, {
						left: g + 38 + "px"
					}).src = "images/Plants/ShroomBulletHit.gif", oSym.addTask(10, ClearChild, [
						d
					])) : (e += 5) < oS.W ? (d.style.left = (g += 5) + "px", oSym.addTask(1, fun, [
						j, d, e, f, g, v
					])) : ClearChild(d);
				}
	...
	oScaredyShroom = InheritO(CPlants, {
		...
		PicArr: function() {
			var a = "images/Plants/ScaredyShroom/",
				b = "images/Plants/";
			return ["images/Card/ScaredyShroom.webp", a + "0.webp", a + "ScaredyShroom.webp", a +
				"ScaredyShroomAttack.webp", a + "ScaredyShroomCry.webp", b + "ShroomBullet.png", b +
				"ShroomBulletHit.gif"
			];
		}(),
		...
		TriggerCheck: function(zombie, direction) {
			var self = this,
				id = self.id;
			let dom = $(id);
			let pic = dom.childNodes[
				1
			]; //zombie.PZ && !zombie.isPuppet && Math.abs(zombie.ZX - self.MX) < 121 && zombie.beAttacked ? (self.ArZ.push(zombie.id), !self.Cry && (self.Cry = 1, $(id).childNodes[1].src = "images/Plants/ScaredyShroom/ScaredyShroomCry.webp", self.CryCheck(id))) : (zombie.R == self.R && !self.Cry && !self.Attacking && zombie.Altitude > 0 && zombie.Altitude < 3 && self.NormalAttack())
			...
			if (zombie.PZ && !zombie.isPuppet && Math.abs(zombie.ZX - self.MX) < 121 && zombie.beAttacked) {
				...
				if (zombie.R == self.R && !self.Cry && !self.Attacking && zombie.Altitude > 0 && zombie
					.Altitude < 3) {
					self.NormalAttack();
				}
			}
		...
		PrivateBirth: function(c) {
			...
			c.BulletEle = NewImg(0, "images/Plants/ShroomBullet.png", "left:" + a + "px;top:" + (c.pixelTop +
				110) + "px;visibility:hidden;z-index:" + (c.zIndex + 2));
			c.MX = b + 9;
		},
		PrivateDie: function(a) {
			a.BulletEle = null;
			...
			oSym.addTask(1, function fun(k, e, f, g, h, u) {
				...
				if (u[g + "_" + j]) {
					...
					i && i.Altitude == 1 ? (i.getPea(i, c.Attack, 0), SetStyle(e, {
						...
						e
					])) : (f += 5) < oS.W ? (e.style.left = (h += 5) + "px", oSym.addTask(1, fun, [
						k, e, f, g, h, u
					])) : ClearChild(e);
				}
		...
		CryCheck: function(id) {
			oSym.addTask(140, function(pid) {
				...
				if (self) {
					...
					while (len--) {
						if (!(zombie = $Z[ArZ[len]]) || !zombie.PZ || Math.abs(zombie.ZX - self.MX) >
							120) {
							ArZ.splice(len, 1);
						}
					}

					ArZ.length ? self.CryCheck(pid) : (self.Cry = 0, $(pid).childNodes[1].src = self
						.PicArr[2]);
				}
			}, [id]);
		}
	...
	oFumeShroom = InheritO(oRadish, {
		...
		PicArr: function() {
			...
			return ["images/Card/FumeShroom.webp", a + "0.webp", a + "FumeShroom.webp", a +
				"FumeShroomAttack.webp", a + "FumeShroomBullet.webp"
			];
		}(),
		AudioArr: ["fume"],
		...
		PrivateBirth: function(b) {
			var a = b.id;
			NewEle(a + "_Bullet", "div", "position:absolute;visibility:hidden;width:406px;height:48px;left:" + b
				.AttackedRX + "px;top:" + (b.pixelTop + 90) +
				"px;background:url(images/Plants/FumeShroom/FumeShroomBullet.webp);z-index:" + (b.zIndex +
					1), 0, EDPZ);
		},
		NormalAttack: function() {
			PlayAudio("fume");
			...
			oEffects.ImgSpriter({
				ele: bullet,
				data: ["0 0", "0 -96px", "0 -240px", "0 -336px", "0 -432px", "0 -528px", "0 -624px",
					"0 -720px", "0 -816px", "0 -864px", "0 -912px"
				],
				frameNum: 11,
				interval: 9,
				callback: SetHidden
	...
	oSporeShroom = InheritO(CPlants, {
		...
		PicArr: function() {
			var a = "images/Plants/SporeShroom/",
				b = "images/Plants/";
			return ["images/Card/SporeShroom.webp", a + "0.gif", a + "SporeShroom.gif", a +
				"SporeShroomAttack.gif", b + "ShroomBullet.png", b + "ShroomBulletHit.gif", a +
				"Effect.webp"
			];
		}(),
		...
		PrivateBirth(obj) {
			const id = obj.id;
			const effect = NewEle(obj.id + "Effect", "div",
				`position:absolute;z-index:${obj.zIndex + 1};width:631px;height:481px;left:${obj.pixelLeft - 200}px;top:${obj.pixelTop - 216}px;background:url(${obj.PicArr[6]}) no-repeat;`,
				0, EDPZ);
			obj.BulletEle = NewImg(0, `images/Plants/ShroomBullet.png`,
				`left:${obj.AttackedLX - 40}px;top:${obj.pixelTop + 95}px;visibility:hidden;z-index:{obj.zIndex + 3};`
			);
			...
			oSym.addTask(1, function fun(f, j, h, c, n, i, m, k, o, g, u) {
				...
				if (u[i + "_" + e] && k != e) {
					...
					d && d.Altitude == 1 ? (d.getPea(d, h, c), SetStyle(j, {
						...
						j
					])) : (n += l = !c ? 5 : -5) < oS.W && n > 100 ? (j.style.left = (o += l) +
						"px", oSym.addTask(1, fun, [f, j, h, c, n, i, m, k, o, g, u])) : ClearChild(
						j);
				}
			}, [b, $(b), a.Attack, 0, a.AttackedLX, a.R, 0, 0, a.AttackedLX - 40, oGd.$Torch, oGd
				.$Obstacle
			]);
		}
	}),
	oBambooUncle = InheritO(CPlants, {
		...
		SunNum: 200,
		coolTime: 15,
		PicArr: ["images/Card/BambooUncle.webp", "images/Plants/BambooUncle/0.gif",
			"images/Plants/BambooUncle/BambooUncle.gif", "images/Plants/CherryBomb/Boom.png"
		],
		AudioArr: ["cherrybomb"],
		...
		PrivateBirth: function(o) {
			oSym.addTask(80, function(id) {
				...
				if (obj) {
					...
					oEffects.ImgSpriter({
						ele: NewEle(boomImgId, "div",
							`position:absolute;overflow:hidden;z-index:${obj.zIndex + 2};width:283px;height:324px;left:${obj.pixelLeft - 80}px;top:${obj.pixelTop - 220}px;background:url(images/Plants/DoomShroom/Boom.webp) no-repeat;`,
							0, EDPZ),
						styleProperty: 'X',
						changeValue: -283,
						frameNum: 10,
					...
					oSym.addTask(20, ClearChild, [NewEle(boomDivId, "div",
						"position:absolute;z-index:257;width:900px;height:600px;left:0;top:0;background:#FFF;opacity:5",
						0, EDPZ)]);
				}
			}, [o.id]);
		}
	...
	oNutBowling = InheritO(CPlants, {
		...
		zIndex: 1,
		Immediately: true,
		PicArr: ["images/Card/WallNut.webp", "images/Plants/WallNut/0.gif",
		...
		PrivateBirth(self) {
			...
			(function fun() {
				...
				if (self.CanAttack && (targZombie = oZ.getZ0(self.AttackedRX, R)) && targZombie.getCrushed(
						self)) {
					...
					switch (targZombie.Ornaments) {
						...
						default:
							//II类防具僵尸，如读报
							targZombie.CheckOrnHP(targZombie, targZombie.id, targZombie.OrnHP, 400, targZombie
								.PicArr, 0, 0, 0);
					}
	...
	oBigWallNutPay = InheritO(oNutBowling, {
		...
		GetDX: self => -Math.floor(self.width * 0.5),
		GetDY: (R, C, arg, birthed) => (birthed ? -73 : -30) + (arg[0] ? -21 : -
			15) //返回植物底座相对格子中点的偏移量，默认可根据是否种在花盆容器中调整
	...
	oBoomNutBowling = InheritO(oNutBowling, {
		...
		PicArr: function() {
			var a = "images/Plants/WallNut/";
			return ["images/Card/BoomNutBowling.webp", a + "1.gif", a + "BoomNutBowling.webp",
				"images/Plants/CherryBomb/Boom.png"
			];
		}(),
	...
	oBegonia = InheritO(CPlants, {
		...
		PrivateBirth(obj) {
			if (obj && obj.isRifter) {
				//补洞逻辑
				let effect = NewImg(`${obj.id}_Frozen`, obj.PicArr[obj.EffectGif],
					`position:absolute;z-index:${obj.zIndex + 2};width:198px;height:113px;left:${obj.pixelLeft - 65}px;top:${obj.pixelTop}px;`,
					EDPZ);
		...
		getHurt: function(e, b, a) {
			...
			(c.HP -= a) < 1 ? c.Die() : c.HP < 500 && c.HurtStatus < 1 && (c.HurtStatus = 1, d.src =
				"images/Plants/Begonia/cracked.gif");
		}
	}),
	oIceAloe = InheritO(oPeashooter, {
		...
		PicArr: (path => ["images/Card/IceAloe.webp", path + "0.webp", path + "IceAloe.webp", path + "Bullet.webp",
			path + "BulletHit.webp", path + "Attack.webp", path + 'rest.webp'
		])("images/Plants/IceAloe/"),
		Tooltip: $__language_Array__["ee08274dbe62b5dd6d0fe6e98cec5528"],
		Story: $__language_Array__["84ceed6f58e8998a586f16e9975c5aeb"],

		PrivateBirth(self) {
			self.BulletEle = NewImg(0, self.PicArr[3],
				`left:${self.pixelLeft + 100}px;top:${self.pixelTop - 10}px;visibility:hidden;z-index:${self.zIndex + 2};`
			);
		},
		...
		CheckLoop(zid, direction) {
			...
			if ($P[pid]) {
				...
				oSym.addTask(700, _ => {
					$P[pid] && ($(pid).childNodes[1].src = self.PicArr[self.NormalGif], oSym.addTask(100,
							_ => self.AttackCheck1(zid, direction)) //此处务必箭头函数，以免this指针丢失
					);
				});
			}
		},
		...
		NormalAttack(zid) {
			...
			oSym.addTask(100, _ => {
				...
				let zomRelativePos = zombieTarget.HeadTargetPosition[zombieTarget.isAttacking] ?
					zombieTarget.HeadTargetPosition[zombieTarget.isAttacking] : zombieTarget
					.HeadTargetPosition[0]; //僵尸和僵尸头部坐标

				let zY = Number.parseInt(zombieTarget.Ele.style.top) + zombieTarget.DivingDepth +
					zomRelativePos.y - height; //僵尸绝对纵坐标

				let zX = Number.parseInt(zombieTarget.Ele.style.left) + zomRelativePos.x - width; //投出时僵尸横坐标

				let zSpeed = !zombieTarget.isAttacking * zombieTarget.Speed * zombieTarget
					.DeltaDirectionSpeed[zombieTarget.FangXiang];
				...
				let bulletShadow = NewEle(`AloeBullet_Shadow_${Math.random()}`, 'div',
					`opacity:0.5;background-size:29px;background-repeat: no-repeat;width:29px;left:${x}px;top:${self.pixelTop + self.height - 10}px;`, {
						className: 'Shadow'
					}, EDPZ);
				...
				(function drawFrame() {
					...
					bullet.style.transform =
						`rotate(${self.getAngle(x, y, lastX, lastY) - defAngle}rad)`;
					...
					if (x >= x2 && y >= zY && vy > 0 || s < 40) {
						bullet && (bullet.src = RandomPic(self.PicArr[4]), bullet.style.left = x - 40 +
							"px", bullet.style.top = y - 20 + "px", bullet.style.transform =
							`rotate(0deg)`, oSym.addTask(70, ClearChild, [bullet]));
	...
	oPepper = InheritO(oPeashooter, {
		...
		PicArr: function() {
			...
			return ["images/Card/Pepper.webp", a + "0.png", a + "Pepper.gif", a + "Bullet.png", a +
				"BulletHit.gif", a + "Attack.gif"
			];
		}(),
		...
		NormalAttack: function(zid) {
			...
			let leftZombie = oZ.getRangeLeftZ(GetX(-2) - 10, oS.W, a.R, false, false,
				heightCheck); //默认检测的是最右边僵尸，但是我还是需要获取最左边僵尸
			...
			if ((leftZombie === null || leftZombie === void 0 ? void 0 : leftZombie.AttackedRX) < a
				.AttackedLX && leftZombie.Altitude != 3) {
				direction = 1;
			...
			oSym.addTask(120, function() {
				PlayAudio(['throw', 'throw2'][Math.round(Math.random() * 1)]);
				let bullet = CustomBullet(oPepperBullet, [a.PicArr[a.BulletGif], a.PicArr[4]], a
					.AttackedLX - direction * 27, a.pixelTop + 40, a.R);
	...
	oMonotropa = InheritO(oPeashooter, {
		...
		PicArr: function() {
			...
			return ["images/Card/Monotropa.webp", a + "0.png", a + "Monotropa.gif", a + "Bullet.png", a +
				"BulletHit.png", a + "Attack.gif"
			];
		}(),
		Tooltip: $__language_Array__["2ee8751b3138f39bc06c7432fbe33134"],
		Story: $__language_Array__["a4ce449f863bc5f398c5b18e3299233f"],
		PrivateBirth: function(a) {
			a.BulletEle = NewImg(0, a.PicArr[3], "left:" + a.AttackedLX + "px;top:" + (a.pixelTop + 45) +
			...
			oSym.addTask(65, function fun(f, j, n, i, k, o, u) {
				...
				if (u[i + "_" + e] && k != e) {
					...
					d && d.Altitude == 1 ? (d["getPea"](d, a.Attack), SetStyle(j, {
						left: o + 28 + "px"
					}).src = a.PicArr[4], oS.StaticCard && (sun = AppearSun(o + 40, a.pixelTop +
						45, 10, 0, GetY(a.R) - (a.pixelTop + 45) - 30 - Math.random() * 10,
						-50, 48, 72), oSym.addTask(100, function() {
						ClickSun(sun.id);
					})), oSym.addTask(10, ClearChild, [j])) : (n += l = 5) < oS.W && n > 100 ? (j
						.style.left = (o += l) + "px", oSym.addTask(1, fun, [f, j, n, i, k, o, u])
					) : ClearChild(j);
				}
	...
	oTorchwood = InheritO(CPlants, {
		...
		beAttackedPointR: 53,
		SunNum: 175,
		PicArr: ["images/Card/Torchwood.webp", "images/Plants/Torchwood/0.webp",
			"images/Plants/Torchwood/Torchwood.webp", "images/Plants/PB10.webp",
	...
	oKiwibeastStrong = InheritO(CPlants, {
		...
		PicArr: function() {
			...
			return ["images/Card/Kiwibeast.webp", a + "0.webp", a + "Kiwibeast_0.webp", a +
				"Kiwibeast_0_attack.webp", a + "Kiwibeast_1.webp", a + "Kiwibeast_1_attack.webp", a +
				"Kiwibeast_2.webp", a + "Kiwibeast_2_attack.webp", a + 'Kiwibeast_growup_1.webp', a +
				'Kiwibeast_growup_2.webp'
			];
		}(),
		BirthStyle: (self, id, ele, style) => {
			EditEle(ele.childNodes[1], {}, {
				...
				switch (true) {
					case _HP < 1650 && t.HurtStatus < 1:
						...
						t.AttackEffect = (left, top) => oEffects.ImgSpriter({
							ele: NewEle(t.id + '_Effect', "div",
								`position:absolute;overflow:hidden;z-index:${this.zIndex + 2};height:65px;width:106px;left:${left + 6}px;top:${top + 146}px;background:url(images/Plants/Kiwibeast/Effect1.png) no-repeat;`,
								0, EDPZ),
							styleProperty: 'X',
							changeValue: -106,
							frameNum: 3,
					...
					case _HP < 1000 && t.HurtStatus < 2:
						...
						t.AttackEffect = (left, top) => oEffects.ImgSpriter({
							ele: NewEle(t.id + '_Effect', "div",
								`position:absolute;overflow:hidden;z-index:${this.zIndex + 2};height:86px;width:183px;left:${left - 22}px;top:${top + 136}px;background:url(images/Plants/Kiwibeast/Effect2.png) no-repeat;`,
								0, EDPZ),
							styleProperty: 'X',
							changeValue: -183,
							frameNum: 4,
		...
		UpDate() {
			...
			oSym.addTask(100, _ => $P[id] && (ele.src = `images/Plants/Kiwibeast/Kiwibeast_${HurtStatus}.webp`, self
				.cAttackCheck()));
		},
		...
		NormalAttack() {
			...
			oSym.addTask(130, _ => {
				if ($P[id]) {
					...
					for (let _R = o.MinR; _R <= MaxR; _R++) {
						...
						oZ.getArZ(MinX, MaxX, _R).forEach(zombie => zombie.Altitude < 2 && (zombie.getHit1(
							zombie, Attack), canBounce && !zombie.isPuppet && zombie.Bounce()));
					}
			...
			oSym.addTask(o.restTime, _ => $P[id] && o.isAttacking && (ele.src =
				`images/Plants/Kiwibeast/Kiwibeast_${o.HurtStatus}.webp`, o.isAttacking = 0, o
				.cAttackCheck()));
		},

		AttackEffect(left, top) {
			oEffects.ImgSpriter({
				ele: NewEle(this.id + '_Effect', "div",
					`position:absolute;overflow:hidden;z-index:${this.zIndex + 2};height:64px;width:118px;left:${left + 8}px;top:${top + 146}px;background:url(images/Plants/Kiwibeast/Effect0.png) no-repeat;`,
					0, EDPZ),
				styleProperty: 'X',
				changeValue: -118,
				frameNum: 5,
	...
	oCabbage = InheritO(oPeashooter, {
		...
		PicArr: function() {
			...
			return ["images/Card/Cabbage.webp", a + "0.gif", a + "Cabbage.gif", a + "Bullet.png", a +
				"BulletHit.gif", a + "CabbageAttack.gif"
			];
		}(),
		...
		PrivateBirth(a) {
			a.BulletEle = NewImg(0, a.PicArr[3], "left:" + (a.pixelLeft + 50) + "px;top:" + (a.pixelTop + 10) +
				"px;visibility:hidden;z-index:" + (a.zIndex + 2));
		},
		...
		NormalAttack(zid) {
			...
			oSym.addTask(70, _ => {
				...
				let [lastX, lastY] = [x, y];
				let defAngle = self.getAngle(x + vx, y + vy + gravity, lastX, lastY);
				let bulletShadow = NewEle(`Cabbage${self.id}_B_${Math.random()}_Shadow`, 'div',
					`opacity:0.5;background-size:29px;background-repeat: no-repeat;width:29px;left:${x}px;top:${self.pixelTop + self.height - 10}px;`, {
						className: 'Shadow'
					}, EDPZ);
				...
				(function drawFrame() {
					...
					bullet.style.top = (y += vy * dt) + 'px';
					bullet.style.transform =
						`rotate(${self.getAngle(x, y, lastX, lastY) - defAngle - 25}deg)`;
					...
					if (x >= x2 && y >= zY && vy > 0 || s < 40) {
						//僵尸距离太小的情况
						bullet && (bullet.src = self.PicArr[4], bullet.style.transform = `rotate(0deg)`,
	...
	oKernelPult = InheritO(oCabbage, {
		...
		PicArr: function() {
			...
			return ["images/Card/KernelPult.webp", a + "kernelpult.webp", a + "idle.webp", a + "attack1.webp",
				a + "attack2.webp", a + "corn.webp", a + "butter.webp", a + "corn_hit.webp", a +
				"butter_hit.webp"
			];
		}(),
		...
		PrivateBirth(a) {
			a.BulletEle = NewImg(0, a.PicArr[5], "left:" + (a.pixelLeft + 50) + "px;top:" + (a.pixelTop + 10) +
				"px;visibility:hidden;z-index:" + (a.zIndex + 2));
			a.ButterEle = NewImg(0, a.PicArr[6], "left:" + (a.pixelLeft + 50) + "px;top:" + (a.pixelTop + 10) +
				"px;height:40;width:auto;visibility:hidden;z-index:" + (a.zIndex + 2));
		...
		NormalAttack(zid) {
			...
			oSym.addTask(55, _ => {
				...
				let bulletShadow = NewEle(`${self.id}_B_${Math.random()}_Shadow`, 'div',
					`opacity:0.5;background-size:29px;background-repeat: no-repeat;width:29px;left:${x}px;top:${self.pixelTop + self.height - 10}px;`, {
						className: 'Shadow'
					}, EDPZ);
				...
				(function drawFrame() {
					...
					if (x >= x2 - 40 && y >= zY && vy > 0 || s < 40) {
						...
						bullet && (bullet.src = self.PicArr[7 + isButter], bullet.style.transform =
							`rotate(0deg)`, oSym.addTask(13, ClearChild, [bullet]));
	...
	oShiitake = InheritO(oPeashooter, {
		...
		BulletHitGif: 5,
		Attack: 200,
		PicArr: (path => ["images/Card/Shiitake.webp", path + "0.webp", path + "idle.webp", path + "attack.webp",
			path + "bullet.webp", path + "bulletHit.webp"
		])("images/Plants/Shiitake/"),
		...
		PrivateBirth(self) {
			self.BulletEle = NewImg(0, self.PicArr[self.BulletGif],
				`top:${self.pixelTop + 12}px;visibility:hidden;z-index:${self.zIndex + 2}`);
		},
	...
	oElecTurnip = InheritO(oSpikeweed, {
		...
		PicArr: function() {
			...
			return ["images/Card/ElecTurnip.webp", a + "0.png", a + "idle.webp", a + "Spawn.webp", a +
				"block.png", a + "Attack.webp"
			];
		}(),
		...
		PrivateBirth: function(self, dd = $(self.id)) {
			...
			oSym.addTask(150, _ => {
				...
				oSym.addTask(190, _ => {
					...
					self.BlockPicDom = NewImg(undefined, self.PicArr[self.BlockGif],
						`position:absolute;left:${self.pixelLeft + 15}px;top:${self.pixelTop + 80}px;z-index:1`,
						EDPZ);
					oEffects.Animate(self.BlockPicDom, 'TurnipBlink', 5, null, null, null,
						'infinite');
				});
				oSym.addTask(213, _ => {
					EditCompositeStyle({
		...
		TriggerCheck: function(zombie, h) {
			...
			if (zombie.PZ) {
				...
				if ((ZLX <= PRX && ZLX >= PLX || ZRX <= PRX && ZRX >= PLX || ZLX <= PLX && ZRX >= PRX) && self
					.AttackCheck2(zombie)) {
		...
		NormalAttack: function(zid) {
			...
			for (let R = 1; R < oS.R; R++) {
				for (let C = 1; C < oS.C; C++) {
					...
					if (((_G$ = G[`${R}_${C}_${o.PKind}`]) === null || _G$ === void 0 ? void 0 : _G$.EName) ==
						"oElecTurnip") {
						att += 45;
					}
				}
		...
		Die: function(ticket = "NONE_TICKET") {
			const list = new Set(['NONE_TICKET', 'JNG_TICKET_ShovelPlant', 'JNG_TICKET_MakeRifterZombie',
				'JNG_TICKET_Gargantuar', 'JNG_TICKET_SuperPower'
			]);
			let self = this;
	...
	oCranberry = InheritO(CPlants, {
		...
		PicArr: function() {
			...
			return ["images/Card/Cranberry.webp", a + "1.webp", a + "2.webp", a + "3.webp", a + "4.webp", a +
				"5.webp", a + "static.webp", a + "Bullet.webp", a + "BulletHit.webp", a + "Die.webp"
			];
		}(),
		...
		PrivateBirth(self) {
			...
			self.BulletEle = NewImg(0, self.PicArr[7], "left:" + self.AttackedLX + "px;top:" + (self.pixelTop +
				15) + "px;visibility:hidden;z-index:" + (self.zIndex + 2));
			self.EleBody = $(id).childNodes[1];
			let date = null;
			self.EleClickArea = NewEle("clicking_cran" + Math.random(), "div",
				`opacity:0;position:absolute;height:60px;width:60px;top:${self.pixelTop + 25}px;left:${self.pixelLeft + self.beAttackedPointL}px;background:blue;z-index:30;`, {},
				EDPZ);
		...
		PrivateDie(c, type = 0, dom) {
			...
			oSym.addTask(100 / 24 * 5, _ => {
				...
				oEffects.ScreenShake();
				let eff = NewImg(0, self.PicArr[9], "left:" + c.pixelLeft + "px;top:" + c.pixelTop +
					"px;z-index:" + (c.zIndex - 2), EDPZ);
		...
		Die: function(ticket) {
			...
			const list = new Set(['JNG_TICKET_MakeRifterZombie', 'JNG_TICKET_Gargantuar',
				'JNG_TICKET_ThiefZombie', 'JNG_TICKET_SuperPower'
			]);
			...
			oSym.addTask(10, function fun(f, j, h, c, n, i, m
				...
				if (u[i + "_" + e] && k != e) {
					...
					d && !d.isGoingDie && d.Altitude == 1 ? (d["getPea"](d, h, c), SetStyle(j, {
						left: o + 10 + "px"
					}).src = a.PicArr[8], oSym.addTask(30, ClearChild, [j])) : (n += l = !c ? spd :
						-spd) < oS.W && n > 100 ? (j.style.left = (o += l) + "px", oSym.addTask(1,
						fun, [f, j, h, c, n, i, m, k, o, g, u])) : ClearChild(j);
				}
			}, [b, $(b), attackV, 0, a.AttackedLX + 30, a.R, 0, 0, a.AttackedLX, oGd.$Torch, oGd
				.$Obstacle
			]);
		},
	...
	oMelonPult = InheritO(oCabbage, {
		...
		PicArr: function() {
			...
			return ["images/Card/MelonPult.webp", b + "0.webp", b + "static.webp", b + "Bullet.webp", b +
				"attack.webp"
			].concat(arr);
		}(),
		...
		PrivateBirth(a) {
			a.BulletEle = NewImg(0, a.PicArr[3], "left:" + (a.pixelLeft + 50) + "px;top:" + (a.pixelTop + 10) +
				"px;width:40px;visibility:hidden;z-index:" + (a.zIndex + 2));
		},
	...
	oAbutilonHybriden = InheritO(CPlants, {
		...
		PicArr: function() {
			...
			return ["images/Card/AbutilonHybriden.webp", a + "0.gif", a + "AbutilonHybriden.gif",
				'images/Zombies/buff_vertigo.webp', a + "Firefly.gif", a + "shadow.png"
			];
		}(),
		...
		PrivateBirth: function(a) {
			oSym.addTask(400, function(b) {
				var e = $P[b],
					d,
					f;
				e && (d = e.R, f = e.C, e.Die(), PlayAudio("AbutilonHybriden"), !oGd.$[d + "_" + f +
					"_1"] && CustomSpecial(oFirefly, d, f));
			}, [a.id]);
		}
	}),
	oFirefly = InheritO(oStoneFlower, {
		...
		BirthStyle: function(c, d, b, a) {
			...
			NewImg(d + "_shadow", "images/Plants/AbutilonHybriden/shadow.png", "left:" + c.pixelLeft +
				"px;top:" + (c.pixelTop + 20) + "px;z-index:" + (c.zIndex - 2), EDPZ);
		},
	...
	oPumpkinHead = InheritO(CPlants, {
		...
		PicArr: function() {
			...
			return ["images/Card/PumpkinHead.webp", a + "0.gif", a + "Pumpkin_back.gif", a +
				"pumpkin_damage1.gif", a + "Pumpkin_damage2.gif", a + "PumpkinHead1.gif"
			];
		}(),
		...
		CanGrow(data, R, C) {
			...
			if ((oGd.$GdType[R][C] === 1 || oGd.$GdType[R][C] === 2 && oGd.$WaterDepth[R][C] === 0 || d0) && !(d2 ||
					C < 1 || C > 9 || oGd.$Crater[flatCoord] || oGd.$LockingGrid[flatCoord])) {
		...
		getHurt: function(e, c, b) {
			...
			switch (true) {
				...
				case d.HP < 1334:
					d.HurtStatus < 2 && (d.HurtStatus = 2, a.childNodes[1].src =
						"images/Plants/PumpkinHead/pumpkin_damage2.gif", d.RefreshImg());
					break;

				case d.HP < 2667:
					d.HurtStatus < 1 && (d.HurtStatus = 1, a.childNodes[1].src =
						"images/Plants/PumpkinHead/pumpkin_damage1.gif", d.RefreshImg());
			}
		},
		InitTrigger: function() {},
	...
	oMiracleImitater = InheritO(oStoneFlower, {
		...
		Die: function(ticket) {
			const self = this,
				whitelist = new Set([`JNG_TICKET_MembraneZombie`, `JNG_TICKET_IceStorm`,
					'JNG_TICKET_Gargantuar', 'JNG_TICKET_MakeRifterZombie', 'JNG_TICKET_ThiefZombie',
					"JNG_TICKET_SuperPower"
				]);
	...
	oJalapeno = InheritO(oCherryBomb, {
		...
		SunNum: 125,
		coolTime: 30,
		PicArr: ["images/Card/Jalapeno.webp", "images/Plants/Jalapeno/0.webp",
			"images/Plants/Jalapeno/Jalapeno.webp", "images/Plants/Jalapeno/JalapenoAttack.webp"
		],
		AudioArr: ["jalapeno"],
		...
		PrivateBirth(self) {
			oSym.addTask(80, id => {
				...
				if (obj) {
					...
					if ($Ice) {
						for (let leftBorder = $Ice[1], $Crater = oGd.$Crater; leftBorder <
							11; leftBorder++) {
					...
					obj.Die('JNG_TICKET_SuperPower');
					let effect = NewImg(fireId, self.PicArr[3],
						`position: absolute;left:130px;top:${obj.pixelBottom - 83}px;z-index:${obj.zIndex + 1}`,
						EDPZ, {
							width: 752,
							height: 103
						});
	...
	oXshooter = InheritO(oPeashooter, {
		...
		PicArr: function() {
			...
			return ["images/Card/Xshooter.webp", a + "idle.webp", a + "Normal.webp", a + "Attack.webp", a +
				"Bullet_Flower.webp", a + "Bullet_Flower_Splash.webp", a + "Bullet_Leaf.webp", a +
				"Bullet_Leaf_Splash.webp"
			];
		}(),
		...
		PrivateBirth: function(a) {
			a.FlowerEle = NewImg(0, a.PicArr[a.FlowerGif], "left:" + a.AttackedLX + "px;top:" + (a.pixelTop +
				15) + "px;visibility:hidden;z-index:" + (a.zIndex + 2));
			a.BulletEle = NewImg(0, a.PicArr[a.LeafGif], "left:" + a.AttackedLX + "px;top:" + (a.pixelTop +
				15) + "px;visibility:hidden;z-index:" + (a.zIndex + 2));
		},
		...
		getHurt: function(zombie, AKind, Attack) {
			...
			o.SetBrightness(o, ele, 1);
			oSym.addTask(10, _ => $P[id] && o.SetBrightness(o, ele, 0));
			!(AKind % 3) ? (zombie && zombie.getHit1(zombie, 80), (o.HP -= Attack / 2) < 1 && o.Die()) : o
				.Die();
		},
		...
		SpecialBullet(g, f, d, b, c, e, point, a, theZ, type, dist = 100) {
			...
			(function(h) {
				...
				oSym.addTask(1, function shoot(n, l, m, k, i, j, speed = 4 * dist / 100, moveType = 1,
					distance = false, hig = false) {
					let tmpX = m,
						tmpY = k;
					...
					if (n > 1000 || k < -40) {
						...
						if ((type === 0 ? k < b - dist || k < 50 : k > b + dist || k > 590) &&
							moveType < 2) {
							moveType = 2;
						}
						...
						if (moveType === 1) {
							...
							if (theZ && $Z[theZ.id]) {
								...
								if (deltaX < 10) {
									m = theZ.ZX;
									k = zhig;
									!j(theZ, AttackType, i) && (hig = true, i.src = g.PicArr[g
										.FlowerGif + 1]);
								}
							} else {
								if (theZ.ZX < m) {
						...
						if (hig !== true) {
							SetStyle(i, {
								transform: $User.LowPerformanceMode ? "" :
									`rotate(${Math.atan2(k - tmpY, m - tmpX)}rad)`
							});
							oSym.addTask(1, shoot, [m, GetR(k + 15), m, k, i, j, speed, moveType,
								distance, hig
							]);
						}
					}
		...
		NormalAttack() {
			var g = this,
				...
				a = function(j, i, h, time = 15, p = point) {
					return j && j.Altitude == 1 ? (j.getPea(j, p, i), oSym.addTask(time, ClearChild, [h]), false) :
						true;
				};
			...
			oSym.addTask(50, _ => {
					...
					if (theZ.ZX - e > 0) {
						(function(h) {
							...
							oSym.addTask(1, function shoot(n, l, m, k, i, j, tmp) {
								theZ.ZX && (tmp = theZ.ZX);
								let [tmpX, tmpY] = [m, k];
								j(oZ.getZ0(n, l), 0, i, 15, 20) ? n > 900 || k < -40 ?
									ClearChild(i) : (SetStyle(i, {
										left: (m += 4) + "px",
										top: (k = k + g.getPosition(m - d, tmp - d)) +
											"px",
										transform: $User.LowPerformanceMode ? "" :
											`rotate(${Math.atan2(k - tmpY, m - tmpX)}rad)`
									}), oSym.addTask(1, shoot, [m + 8, GetR(k + 15), m, k,
										i, j, tmp
									])) : i.src = g.PicArr[g.LeafGif + 1];
...

				{
					...
					let rand = [
						[triggerR[0], triggerR[1], 1],
						[triggerR[1], triggerR[0], -1]
					].random();
	...
	oFlowerPot = InheritO(CPlants, {
		...
		HP: 1000,
		PicArr: ["images/Card/FlowerPot.webp", "images/Plants/FlowerPot/FlowerPot.png",
			"images/Plants/FlowerPot/FlowerPot.png"
		],
		PKind: 0,
		Stature: -1,
		...
		CanGrow(plantArgs, R, C) {
			let flatCoord = R + "_" + C;
			return oGd.$GdType[R][C] != 2 && !(!oGd.$GdType[R][C] || C < 1 || C > 9 || plantArgs[1] || plantArgs[
				2] || plantArgs[0] || oGd.$Crater[flatCoord] || oGd.$LockingGrid[flatCoord]);
		},
	...
	oLilyPad = InheritO(oFlowerPot, {
		...
		CanGrow(plantArgs, R, C) {
			let flatCoord = R + "_" + C;
			return !(oGd.$GdType[R][C] != 2 || C < 1 || C > 9 || plantArgs[0] || oGd.$Crater[flatCoord] || oGd
				.$LockingGrid[flatCoord]);
		},
		...
		PrivateBirth: function(a) {
			oSym.addTask(40, function(b) {
				var e = $P[b],
					...
					d,
					f;
				e && (d = e.R, f = e.C, e.Die(), oS.StaticCard && AppearSun(GetX(f) - Math.random() *
					41, GetY(d), 300, 0));
			}, [a.id]);
		}
	}),
	...
	olSPCase = InheritO(CPlants, {
		...
		CanGrow(data, R, C) {
			...
			if ((oGd.$GdType[R][C] === 1 || oGd.$GdType[R][C] === 2 && oGd.$WaterDepth[R][C] === 0 || data[0]) && !(
					C < 1 || C > 9 || oGd.$Crater[flatCoord] || oGd.$LockingGrid[flatCoord])) {
		...
		Die(ticket) {
			const list = new Set(['JNG_TICKET_lSPCase', "JNG_TICKET_ThiefZombie", 'JNG_TICKET_ShovelPlant',
				'JNG_TICKET_IceStorm', 'JNG_TICKET_Gargantuar', 'JNG_TICKET_MakeRifterZombie',
				"JNG_TICKET_ThiefZombie", "JNG_TICKET_SuperPower"
			]);
	...
	o8BitApple = InheritO(oStoneFlower, {
		...
		PicArr: function() {
			var a = "images/Plants/Macintosh/";
			return ["", a + "Little_Born.webp", a + "Little_Normal.webp", a + "Little_Attack.webp", a +
				"Little_Die.webp"
			];
		}(),
		...
		NormalAttack: function(zid) {
			...
			let zombie = $Z[zid],
				o = this,
				pid = o.id;
			!o.isAttacking && ($(pid).childNodes[1].src = o.PicArr[3], o.isAttacking = 1, oSym.addTask(128,
				function fun() {
					if ($P[pid]) {
						o.ArZ.length < 1 ? ($(pid).childNodes[1].src = o.PicArr[2], o.isAttacking = 0) :
							oSym.addTask(128, fun);
					}
				}));
			zombie.getHit2(zombie, o.Attack, 0);
		},
		...
		Die(ticket) {
			...
			oSym.addTask(100, function() {
				...
				zombies.forEach(zombie => zombie.Altitude < 2 && zombie.getHit2(zombie, self.Attack * 10,
					0));
			});
	...
	oMissile = InheritO(oLight, {
		...
		Boom: function(obj) {
			...
			oEffects.ScreenShake();
			oEffects.ImgSpriter({
				ele: NewEle(obj.id + '_Boom', "div",
					`position:absolute;overflow:hidden;z-index:${obj.zIndex};width:196px;height:259px;left:${obj.pixelLeft - 78}px;top:${obj.pixelTop - 120}px;background:url(images/Zombies/Boom.png) no-repeat;`,
					0, EDPZ),
				styleProperty: 'X',
				changeValue: -196,
				frameNum: 19,
		...
		PrivateBirth: function(obj, id, ele, pixelTop, targetTop) {
			...
			(function move() {
				pixelTop < targetTop ? $P[id] && (ele.style.top = (pixelTop = obj.pixelTop = pixelTop +
					cv++) + 'px', oSym.addTask(1, move)) : $P[id] && (oGd.add(obj, obj.R + "_" + obj.C +
					"_" + obj.PKind), obj.Boom(obj));
			})();
		}
	}),
	oSummonZombieObs = InheritO(CPlants, {
		...
		PrivateBirth: function(o) {
			...
			NewImg(o.id + "_Light", o.PicArr[0], `clip: rect(78px,auto,auto,auto); top: -78px; opacity: 0;`, $(o
				.id)); //初始化发光特效
		},
		...
		getTriggerRange: function(a, b, c) {
			return [
				[this.pixelLeft + 40, this.pixelRight + 40, 0]
			];
		},
		TriggerCheck: function(i, h) {
			var c = i.id,
				...
				f;
			i.PZ && !g[c] && (a = i.AttackedLX, b = i.AttackedRX, e = this.AttackedLX, f = this.AttackedRX, a <=
					f && a >= e || b <= f && b >= e || a <= e && b >= f) && this.AttackCheck2(i) && this
				.NormalAttack(c);
		},
		AttackCheck2: function(a) {
			//触发特殊条件检查器
	...
	oRifter = InheritO(oObstacle, {
		...
		Birth(X, Y, R, C, plantsArg, isTyped = false) {
			...
			NewImg(0,
				`images/Props/Rifter/${isTyped !== false ? isTyped : 1 + Math.round(Math.random() * 3)}${oS.DKind ? '' : 'dark'}.png`,
				'clip:rect(auto, 80px, auto, auto);', ele);
			self.InitTrigger(self, id, self.R = R, self.C = C, self.AttackedLX = pixelLeft + self
				.beAttackedPointL, //植物左检测点
				self.AttackedRX = pixelLeft + self.beAttackedPointR //植物右检测点
			);
	...
	oRifterAnimate = InheritO(oObstacle, {
		...
		CanGrow: function(c, b, d) {
			let a = b + "_" + d;
			return c[2] ? 1 : oGd.$GdType[b][d] == 1 ? !(d < 1 || d > 9) && (c[1] ? c[1].canShovel : true) : c[
				0];
		},
		PicArr: function() {
		...
		Birth(X, Y, R, C, plantsArg) {
			...
			NewImg(0,
				`images/Props/Rifter/${self.iceType = 1 + Math.round(Math.random() * 3)}${oS.DKind ? '' : 'dark'}.png`,
				'opacity:0.6;left:-80px;clip:rect(auto, 160px, auto, 80px);', ele);
			self.InitTrigger(self, id, self.R = R, self.C = C, self.AttackedLX = pixelLeft + self
				.beAttackedPointL, //植物左检测点
				self.AttackedRX = pixelLeft + self.beAttackedPointR //植物右检测点
			);
			...
			if (self.Pepper && oGd.$[`${R}_${C}_3`] || oGd.$Crater[R + '_' + C] || oGd.$[`${R}_${C}_1`] && !oGd.$[
					`${R}_${C}_1`].canShovel) {
				self.GoDie = true;
			} else {
				oGd.add(self, `${R}_${C}_${self.PKind}`); //在场景注册
						...
						if (!num) {
							...
							if (hasP = oGd.$[`${self.R}_${self.C}_1`]) {
								...
								if (oGd.$[`${self.R}_${self.C}_1`].EName != "oBegonia") {
									oEffects.ImgSpriter({
										ele: NewEle(`${obj.id}_Drop`, "div",
											`position:absolute;overflow:hidden;z-index:${obj.zIndex + 2};width:150px;height:184px;left:${obj.pixelLeft - 30}px;top:${obj.pixelTop - 60}px;transform:scale(1.3);background:url(images/Props/Rifter/Drop_Water.png) no-repeat`,
											0, EDPZ),
										styleProperty: 'X',
										changeValue: -150,
										frameNum: 37,
							...
							if (hasP && oGd.$[`${self.R}_${self.C}_1`].EName != "oBegonia" || !hasP) {
								...
								new oRifter().Birth(GetX(self.C), GetY(self.R), self.R, self.C, [], self
									.iceType);
							} else if (hasP) {
								hasP.Die();
							}
		...
		NormalAttack: function(id, a) {
			...
			if (zombie.R != o.CantGo && zombie.AKind != 2 && oGd.$Crater[o.R + '_' + o.C] != 1) {
				...
				oEffects.Animate(dom_, {
					...
					dom_.style.cssText +=
						`top:${zombie.pixelTop}px;z-index:${zombie.zIndex = 3 * value + 1};`;
					oEffects.Animate(o.EleBody2, {
						opacity: 0
					}, 0.2 / oSym.NowSpeed);
				});
	...
	oGoDown = InheritO(oStoneFlower, {
		...
		TriggerCheck(zombie) {
			if (zombie.FangXiang !== 'GoDown') {
				...
				$User.LowPerformanceMode === false && zombie.FangXiang !== 'GoDown' && (EDPZ_Spare2.insertBefore(
					zombie.Ele, EDPZ_Spare2.children[0]), zombie.Wrap = 'dPZ_Spare2');
		...
		getTriggerRange(R, LX, RX) {
			return [
				[GetX(this.C) - 50, GetX(this.C) - 40, 0]
			];
		},
	...
	oGoDownFixed = InheritO(oGoDown, {
		...
		getTriggerRange(R, LX, RX) {
			return [
				[GetX(this.C) + 30, GetX(this.C) + 35, 0]
			];
		}
	...
	oGoUp = InheritO(oGoDown, {
		...
		TriggerCheck(zombie) {
			if (zombie.FangXiang !== 'GoUp') {
				...
				$User.LowPerformanceMode === false && zombie.FangXiang !== 'GoUp' && (EDPZ_Spare1.append(zombie
					.Ele), zombie.Wrap = 'dPZ_Spare1');
	...
	oGoRight = InheritO(oGoDown, {
		...
		TriggerCheck(zombie) {
			if (zombie.FangXiang !== 'GoRight') {
				...
				zombie.EleBody['style']['transform-origin'] =
					`${(zombie.beAttackedPointL + zombie.beAttackedPointR) / 2}px 0px`;
			}
		}
	...
	oVase = InheritO(oObstacle, {
		...
		PrivateBirth: function(self) {
			...
			function callback() {
				self.EleClickArea = NewEle("clicking" + Math.random(), "div",
					`cursor:pointer;opacity:0;position:absolute;height:80px;width:70px;top:${self.pixelTop + 30}px;left:${self.pixelLeft + self.beAttackedPointL + 5}px;background:blue;z-index:30;`, {},
					EDPZ);
		...
		Break_Vase(self) {
			oEffects.ImgSpriter({
				ele: NewEle(`${self.id}_Break`, "div",
					`position:absolute;overflow:hidden;z-index:${self.zIndex};width:141px;height:136px;left:${self.pixelLeft}px;top:${self.pixelTop}px;background:url(images/Props/Vase_Breaker/Pop_Effect.webp) no-repeat`,
					0, EDPZ),
				styleProperty: 'X',
				changeValue: -141,
				frameNum: 18,
		...
		SetCard(config = {}) {
			...
			function callback() {
				self.configs = config;
				self.EleCard.src = !config.type ? pro.PicArr[pro.CardGif] : "images/Card/" + pro.EName.substring(1)
					.split("_")[0] + ".webp";
		...
		FallDown: function(self, callback) {
			...
			function stop() {
				...
				if (oGd.$GdType[self.R][self.C] === 2) {
					self.EleShadow.style.cssText +=
						`background:url(${self.PicArr[self.WaterGif]});height:91px;width:260.5px;background-size:100% 100%;z-index:300;`;
					EditCompositeStyle({
						ele: self.EleShadow,
						addFuncs: [
							["translate", "-87.25px,-25px"]
						],
						option: 2
					});
					SetStyle(self.EleBody, {
	...
	oTrafficLight = InheritO(oObstacle, {
		...
		width: 67,
		AutoChange: true,
		PicArr: ["images/Card/AbutilonHybriden.webp", "images/Props/TrafficLight/red.png",
			"images/Props/TrafficLight/green.png", "images/Props/TrafficLight/close.png"
		],

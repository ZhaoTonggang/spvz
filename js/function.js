...
var asyncInnerHTML = function(HTML, callBack, arg) {
	...
	DisplayZombie = function(left = 50, top = 150, width = 201, height = 444) {
		...
		AZ.forEach((zombie, index) => {
			...
			htmls.push(proto.getDisplayHTML("", Math.floor(left + Math.random() * width) - proto.width * 0.5,
				randoms[index] - ((_proto$displayHeight = proto.displayHeight) !== null &&
					_proto$displayHeight !== void 0 ? _proto$displayHeight : proto.height), 1, "block",
				"auto", proto.GetDTop, proto.PicArr[proto.StandGif]));
		}); //渲染dom
	...
	InitPCard = function() {
		...
		oS.ArCard.forEach(plant => {
			...
			html +=
				`<div class="aCard" id="Card${EName}" onmouseout="SetHidden($('dTitle'))" onmousemove="ViewCardTitle(${EName}, event)" onmousedown="SelectCard('${EName}')"><img src="${proto.PicArr[0]}"><span class="cardSunNum">${proto.SunNum}</span></div>`;
		});
	...
	GetAP = (evtX, evtY, R, C) => {
		...
		for (let Z = 0; Z <= PKindUpperLimit; Z++) {
			...
			if (plant && evtX > plant.pixelLeft && evtX < plant.pixelRight && evtY > plant.pixelTop && evtY < plant
				.pixelBottom) {
				pointPlant = plant;
			}
		}
	...
	GroundOnmousedown = function(event) {
		...
		let [
			[X, C],
			[Y, R]
		] = [ChosePlantX(evtX), ChosePlantY(evtY)];
	...
	GroundOnmousemove2 = function() {
		...
		if (MPID !== ID) {
			...
			(oS.MPID = ID) && $P[ID].canShovel && newPointedEle && SetAlpha(newPointedEle.childNodes[1],
				0.6); //改变后一植物的透明度
		}
	...
	ChoseShovel = function(event) {
		...
		ClearChild($("tShovel"));
		SetHidden($('imgShovel'));
		NewImg('tShovel', 'images/interface/Shovel.png',
			`left:${$User.clientX - 20}px;top:${$User.clientY - 38}px;z-index:258;`, FightingScene);
	...
	ViewCardTitle = function(plant, event) {
		...
		let proto = plant.prototype;
		let str =
			`${proto.CName}${$__language_Array__["96922b1b075633116839cab7596bdabc"][0]}${proto.coolTime}${$__language_Array__["96922b1b075633116839cab7596bdabc"][1]}${proto.Tooltip}`;
		let obj;
		...
		if (obj = InLockedCards(proto.EName)) {
			...
			str +=
				`${$__language_Array__["dba01bde7e595e8239e4ae4ff0d1687e"][0]}${str2}${$__language_Array__["dba01bde7e595e8239e4ae4ff0d1687e"][1]}`;
		}

		ele.innerHTML = str;
		SetStyle(ele, {
			left: event.clientX - EDAlloffsetLeft + 385 - 3 + "px",
			top: event.clientY - EDAlloffsetTop + 18 + "px",
			visibility: "visible"
		});
	},
	ViewPlantTitle = function(index) {
		...
		let str =
			`${plant.CName}${$__language_Array__["96922b1b075633116839cab7596bdabc"][0]}${plant.coolTime}${$__language_Array__["96922b1b075633116839cab7596bdabc"][1]}${plant.Tooltip}`;
	...
	BeginCool = function() {
		...
		ArCard.forEach((card, index) => {
			...
			let maskFather = NewEle(`dCDFather${card.DID}`, "div", "pointer-events:none;position:relative;",
				null, $(card.DID));
			let borderStr = "20px";
			...
			let mask = NewEle(`dCDMask${card.DID}`, "div",
				`background-color:black;border-radius:${borderStr};position:relative;opacity:0;`, null,
				maskFather);
	...
	ChosePlant = function(evt, index) {
		...
		let evtX = evt.clientX - EDAlloffsetLeft,
			evtY = evt.clientY - EDAlloffsetTop,
		...
		if (!oS.CardKind) {
			//选择的是植物卡片
			let MovePlant = NewImg("MovePlant", PicArr[Pro.StaticGif],
				`left:${evtX}px;top:${evtY + 20 - Pro.height}px;z-index:258;`, FightingScene);
			...
			//选择的是僵尸卡片
			NewImg("MovePlant", Pro.PicArr[Pro.StandGif], "left:" + (evtX - 0.5 * (Pro.beAttackedPointL + Pro
				.beAttackedPointR)) + "px;top:" + (evtY + 20 - Pro.height) + "px;z-index:258", FightingScene);
			NewImg("MovePlantAlpha", Pro.PicArr[Pro.StandGif], "visibility:hidden;opacity:0.4;z-index:30",
				FightingScene);
		}
	...
	AutoProduceSun = function(num) {
		//这是关卡自动掉落阳光的函数
		oS.DKind && AppearSun(GetX(Math.floor(1 + Math.random() * oS.C)), GetY(Math.floor(1 + Math.random() * oS.R)),
			num, 1);
	...
	MoveClickSun = function(id) {
		...
		SetStyle($(id), {
			...
			opacity: 0.75
		}).addEventListener('transitionend', _ => ArSun[id] && (oS.SunNum += obj.N, MonitorCard(), delete ArSun[id],
			ClearChild(ele)));
	},
	...
	PrepareGrowPlants = function(fun, anim = true, text = [
		[$__language_Array__["b111bd4e7d0cfef528a39240f66972ba"], 60, "80px"],
		[$__language_Array__["51b864cd94837d49493573d3c15a1450"], 40, "60px"],
		[$__language_Array__["c130f856ce35cdff29aaa7fe1d010126"], 100, "90px"]
	]) {
		$("dZombie").innerHTML = '';
		...
		let dom = NewEle("", "div",
			"left: 450px; top: 300px; transform: translate(-50%, -50%);color:red;font-size:80px;opacity:0;", {
				innerText: text[0][0],
				id: "dReadySetPlant",
	...
	CheckAutoSun = function(ele) {
		...
		shifouKaiqi != $User.AutoSun && ($User.AutoSun = localStorage['JNG_TR_AUTOSUN'] = shifouKaiqi, shifouKaiqi &&
			AutoClickSun());
	},
	...
	oFog = {
		...
		render() {
			...
			let createImg = (id, x, y) =>
				`<img id='Fog${id}' class='${this.type} show' src='images/interface/fog${Math.floor(Math.random() * 4)}.png' style='left:${x}px;top:${y}px;'>`; //从上至下，从左至右绘制迷雾
		...
		attackPlants() {
			...
			(function fun() {
				...
				if (!oFog.hasLeftStage && dom === $("dFog")) {
					for (let plant of $P) plant.C >= 10 - oFog.leftBorder && plant.EName !== 'oPlantern' && plant
						.getHurt(null, 0, 5);
	...
	ThrowACard = function(plant, pos, callback, config = {}) {
		...
		const styles =
			`top:${pos[1]}px;left:${pos[0]}px;transform:scale(0);width:100px;height:120px;cursor:pointer;clip:rect(auto,auto,60px,auto);z-index:253;`;
		let card = {
			DID: id,
			PName: obj,
			hasAnim: false,
		...
		(function moveCard() {
			(function() {
				...
				if (card.hasOwnProperty('countdown')) {
					...
					if (((_oS2 = oS) === null || _oS2 === void 0 ? void 0 : (_oS2$ChoseCardObj = _oS2
								.ChoseCardObj) === null || _oS2$ChoseCardObj === void 0 ? void 0 :
							_oS2$ChoseCardObj.DID) !== card.DID) {
						card.countdown -= 3;
						card.countdown <= 300 && !card.hasAnim && (oEffects.Animate(card.Ele, 'CardBlink',
							'slow', null, null, null, 'infinite'), card.hasAnim = true);
		...
		function ChosePlant(evt, obj) {
			...
			let evtX = evt.clientX - EDAlloffsetLeft;
			let evtY = evt.clientY - EDAlloffsetTop;
			...
			GroundOnmousemove = function(event) {
				...
				let evtY = event.clientY - EDAlloffsetTop;
				let [
					[X, C],
					[Y, R]
				] = [ChosePlantX(evtX), ChosePlantY(evtY)];
	...
	toWin = () => {
		...
		NewEle("DivA", "div",
			"position:absolute;width:900px;height:600px;background:#FFF;opacity:0;z-index:257;left:115px;", 0, EDAll
		);
	...
	toOver = function(type) {
		...
		oEffects.Animate(NewEle(Math.random(), "div",
			"position:absolute;width:112px;height:46px;left:428px;top:646px;z-index:310;", {
	...
	oPropSym = {
		...
		readData() {
			...
			this.data = JSON.parse((_localStorage$JNG_TR_ = localStorage.JNG_TR_PurchasedProps) !== null &&
				_localStorage$JNG_TR_ !== void 0 ? _localStorage$JNG_TR_ : "{}");
		},
		...
		isPropUnlocked(name) {
			...
			let unlocks = JSON.parse((_localStorage$JNG_TR_2 = localStorage.JNG_TR_PropsUnlock) !== null &&
				_localStorage$JNG_TR_2 !== void 0 ? _localStorage$JNG_TR_2 : "{}");
			return !!unlocks[name];
		}
	...
	oPropSelectGUI = {
		Init() {
			...
			if (oPropSym.data.length <= 0 || ((_oS3 = oS) === null || _oS3 === void 0 ? void 0 : _oS3.FixedProps
		...
		show() {
			...
			if (oPropSym.data.length <= 0 || ((_oS4 = oS) === null || _oS4 === void 0 ? void 0 : _oS4.FixedProps
					.length) > 0) return this;
		...
		hide() {
			...
			if (oPropSym.data.length <= 0 || ((_oS5 = oS) === null || _oS5 === void 0 ? void 0 : _oS5.FixedProps
					.length) > 0) return this;
		...
		render() {
			...
			while (index <= 1) {
				...
				let ename = (_propNames$index = propNames[index]) !== null && _propNames$index !== void 0 ?
					_propNames$index : 'addIcon';
		...
		renderIcon(index, ename) {
			...
			if (ename === 'addIcon') {
				...
				innerHTML +=
					`<img class="dFrontIcon" src='${oPropSym.staticInforms[ename].img}' height="77" width="77"><span class="dNumText">×${this.PropsJSON[ename]}</span>`;
		...
		clickPropIcon(index) {
			...
			if (GetStyle(dom, 'visibility') === 'hidden') {
				...
				this.wrapDom.querySelector(`#IconWrap_${index}`).dataset['jngEname'] === 'addIcon' ? SetNone($(
					'PropSelect_ListIcon_addIcon')) : SetBlock($('PropSelect_ListIcon_addIcon'));
			} else {
				SetHidden(dom);
			}
	...
	oPropUseGUI = {
		Init() {
			...
			if (((_oS6 = oS) === null || _oS6 === void 0 ? void 0 : _oS6.FixedProps.length) > 0 && ((_oS7 = oS) ===
					null || _oS7 === void 0 ? void 0 : _oS7.FixedProps) !== 'disabled') {
				this.PropsJSON = oS.FixedProps;
			} else if ((list === null || list === void 0 ? void 0 : list.size) > 0 && ((_oS8 = oS) === null || _oS8 ===
					void 0 ? void 0 : _oS8.FixedProps) !== 'disabled') {
			...
			} else if ((oPropSym === null || oPropSym === void 0 ? void 0 : oPropSym.data.length) > 0 && !list && ((
					_oS9 = oS) === null || _oS9 === void 0 ? void 0 : _oS9.FixedProps) !== 'disabled') {
		...
		updateNums() {
			...
			if (((_oS10 = oS) === null || _oS10 === void 0 ? void 0 : _oS10.FixedProps.length) > 0 && ((_oS11 = oS) ===
					null || _oS11 === void 0 ? void 0 : _oS11.FixedProps) !== 'disabled') {
				//如果是固定道具就不用读取存档
				return;
			}
		...
		*/
		render() {
			...
			const DOM_BottomBar = getBorderCanvasDom("images/Props/Shop/buttonSlot.png", 10, 10, PropKindNum * 92 + 15,
				21);
			EditEle(DOM_BottomBar, null, null, propsDom, {
				id: 'dBottomBar',
			...
			for (let ename in PropsJSON) {
				...
				iconWrap.innerHTML =
					`<img class="dLatterIcon" src="images/Props/Shop/button.png"><canvas class="dCoolIcon" height="77" width="77"></canvas><img class="dFrontIcon" src='${oPropSym.staticInforms[ename].img}'><span class="dNumText">×${obj.num === Infinity ? '∞' : obj.num}</span>`; //前置图标（按钮）
		...
		drawCoolTimeCircle(ctx, circle) {
			...
			ctx.scale(-1, 1);
			ctx.translate(-circle.x, -circle.y);
			ctx.arc(circle.x, circle.y, circle.r / 2, Math.PI * 3 / 2, Math.PI * 3 / 2 + Math.PI * 2 * circle.percent,
				false);
			ctx.stroke();
			ctx.restore();
		},
	...
	GetWin = function(img, lvl, callback) {
		...
		oEffects.fadeIn($("DivA"), 6, _ => {
			/(Forest|Marsh|Polar|Industry)_SelectionMap_\d/.test(lvl) ? (oSelectionMap._CurrentPage_ = lvl,
				SelectModal('selectionmap', 'Service')) : SelectModal(lvl);
			callback && callback();
		}, [0, 100]);
	},
	//抛出物品
	...
	GetNewProp = function(a, p, t, s, lvl, o, k) {
		...
		oEffects.fadeIn($("DivA"), 6, function() {
			...
			NewProp.style.backgroundImage = /\.\w+/.test(p) ? "url(" + p + ")" : "url(images/interface/" + p +
				".png)";
			...
			$("NextLevel").onclick = function() {
				StopAudio("plantsgarden");
				SetHidden($("dNewProp"));
				/(Forest|Marsh|Polar|Industry)_SelectionMap_\d/.test(lvl) ? (oSelectionMap._CurrentPage_ =
					lvl, SelectModal('selectionmap', 'Service')) : SelectModal(lvl);
				PlayAudio("pause");
			};
	...
	PlaceZombie = function(constructor, R, C, animation = true, sync = false) {
		...
		if (animation) {
			PlayAudio('dirt_rise');
			oEffects.ImgSpriter({
				ele: NewEle(`Dirt_${Math.random()}`, "div",
					`position:absolute;background:url(images/Zombies/dirt.png) no-repeat;z-index:${3 * R + 2};left:${755 - (9 - C) * 80}px;top:${100 * (R - 1) + 40}px;height:162px;width:126px;`,
					0, EDPZ),
				styleProperty: 'X',
				changeValue: -126,
				frameNum: 22,
	...
	hasPlants = function(basic = true, filterFn) {
		...
		for (let plant of $P) {
			let basicResult = basic ? plant.EName !== 'oBrains' && plant.EName !== 'oLawnCleaner' && plant.EName !==
				'oApple' && plant.canShovel : true,
				//基础查询
	...
	oMiniGames = {
		...
		ProtectPlants(sheet, sheetPos = []) {
			...
			PrepareGrowPlants(function() {
				...
				(function fun() {
					...
					for (let o of $P) recordSheet.hasOwnProperty(o.EName) && positions.has(
						`${o.R}_${o.C}`) && ++recordSheet[o.EName];
		...
		*/
				...
				(function moveCard() {
					...
					while (len--) {
						...
						card.PixelTop > 60 * len + 5 //卡牌没有到达顶端
							...
							&&
							($(card.DID).style.top = (card.PixelTop = Math.max(7, 60 * len + 7, card
								.PixelTop - 1)) + "px");
					}
		...
		RainWithSeeds(arrP = oS.PName, createTime = 600, moveTime = 5, priorityP = [], countdownTime = 700,
			autoRainEffect = true, maxCard = 9) {
			/* 一些初始化 */
			StopMusic();
			...
			PrepareGrowPlants(() => {
				...
				(function createNewCard() {
					...
					if (len < maxCard) {
						if ((_priorityP === null || _priorityP === void 0 ? void 0 : _priorityP[index]) ===
							"null") {
							index++;
						} else {
							...
							const styles =
								`top:-60px;left:${100 + Math.random() * 600}px;width:100px;height:120px;cursor:pointer;clip:rect(auto,auto,60px,auto);z-index:253;`;
				...
				(function moveCard() {
					ArCard.forEach(card => {
						if (card.PixelTop <= card.EndTop) {
							card.Ele.style.top = (card.PixelTop += Math.max((75 - card.PixelTop) /
								20, 1)) + "px";
						...
						if (card.hasOwnProperty('countdown')) {
							...
							if (((_oS12 = oS) === null || _oS12 === void 0 ? void 0 : (
										_oS12$ChoseCardObj = _oS12.ChoseCardObj) === null ||
									_oS12$ChoseCardObj === void 0 ? void 0 : _oS12$ChoseCardObj.DID
								) !== card.DID) {
								card.countdown -= moveTime;
								card.countdown <= 300 && !card.hasAnim && (oEffects.Animate(card
									.Ele, 'CardBlink', 'slow', null, null, null, 'infinite'
								), card.hasAnim = true);
		...
		DarkRain(time = 12, randomRange = 10, thunderDelay = 4) {
			//黑暗暴雨
			let FlashMask = NewEle(`Flash_Mask${Math.random()}`, "div",
				"pointer-events:none;position:absolute;z-index:31;left:0px;width:1600px;height:600px;background:#000;opacity:0.4;",
				0, EDAll);
			let flash_on = 1;
			let Mask = NewEle(`Mask${Math.random()}`, "div",
				"pointer-events:none;position:absolute;z-index:31;left:0px;width:1600px;height:600px;background:#000;opacity:0.5;",
				0, EDAll);
			oEffects.BgParticle({
				style: "z-index:21",
				url: "images/Props/Effect/Rain.png",
			...
			oSym.addTask(150, function() {
				...
				oS.isStartGame === 1 ? thunderStart() : addEventListenerRecord('jng-event-startgame',
					thunderStart);
			});
		...
		LimitedTimeNoCool(time = Infinity, fun, timerBar = null) {
...

			{
				let text = [
					[$__language_Array__["b111bd4e7d0cfef528a39240f66972ba"], 60, "80px"],
					[$__language_Array__["801098e95403158775a49b22bc849d7c"], 40, "60px"],
					[$__language_Array__["0f1598edc43b823f317e649fa27ee936"], 100, "90px"]
				];
				let dom = NewEle("", "div",
					"left: 450px; top: 300px; transform: translate(-50%, -50%);color:red;font-size:80px;opacity:0;", {
						innerText: text[0][0],
				...
				oSym.addTask(30, function() {
					...
					oSym.addTask(200, function() {
						...
						for (let obj of ArCard) {
							...
							p.SunNum <= oS.SunNum && (obj.SunReady = 1, $(obj.DID).childNodes[0]
								.style.top = "0");
						...
						if (time !== Infinity) {
							oSym.addTask(1, function loop() {
								...
								let curValue = timerBar ? timerBar(time / oldTime) : (
									time / oldTime) ** 1.1;
			...
			function FIGHT(timeLimited = false) {
				...
				for (let obj of ArCard) {
					obj.PName.prototype.SunNum > oS.SunNum && (obj.SunReady = 0, $(obj.DID).childNodes[0].style.top =
						"60px");
					obj.CDReady = 0;
		...
		GrowWithoutSun(AutoControl = true, prepareFun) {
			...
			if (AutoControl) {
				...
				PrepareGrowPlants(_ => {
					...
					for (let obj of ArCard) {
						obj.PName.prototype.SunNum <= oS.SunNum && (obj.SunReady = 1, $(obj.DID).childNodes[0]
							.style.top = "0");
						obj.CDReady = 1; //标记冷却完成
					}
					...
					if (oS.Chose < 1) {
						while (b--) {
							if (!(d = ArCard[b]).Forbidden) {
								//没有在争分夺秒里被禁用的植物
								(c = d.PName.prototype).SunNum > oS.SunNum && !d.Forbidden ? (d.SunReady && (d
									.SunReady = 0), $(d.DID).childNodes[0].style.top = "-60px") : (!d
									.SunReady && (d.SunReady = 1), $(d.DID).childNodes[0].style.top = 0);
							}
						}
					} else {
						while (b--) {
							if (!(d = ArCard[b]).Forbidden) {
								//没有在争分夺秒里被禁用的植物
								(c = d.PName.prototype).SunNum > oS.SunNum ? d.SunReady && (d.SunReady = 0) : !d
									.SunReady && (d.SunReady = 1);
							}
						}
					}
					...
					requestAnimationFrame(_ => {
						...
						oSym.addTask(200 * Math.random(), () => {
							oEffects.Animate(obj, {
								...
								}, (Math.random() / 2 + 0.5) / oSym.NowSpeed, "ease-out",
								ClearChild);
						});
					});
				}
		...
		Frostbite(apartTime, hurt) {
			...
			function _Frostbite() {
				//定时降温
				PlayAudio('frostbite');
				oEffects.Animate(NewEle(`Frostbite${Math.random()}`, "div",
		...
		*/
		WinWithScore({
			...
			addEventListenerRecord('jng-event-startgame', () => {
				new MutationObserver(mutations => IsGaming(1) && mutations.forEach(MutationRecord =>
					MutationRecord.removedNodes.forEach(markScroe))).observe(EDPZ, {
					childList: true
				});
			});
		},
		...
		oStg: {
			...
			checkTrigger(trigger1, trigger2) {
				//圆形判定
				const res = Math.pow(trigger1.x - trigger2.x, 2) + Math.pow(trigger1.y - trigger2.y, 2) <= Math.pow(
					trigger1.r + trigger2.r, 2);
				return res;
			},
			...
			Init() {
				...
				if (Mobile && /file:/.test(location.href) || localStorage.JNG_DEV == "true" && /file:/.test(location
						.href)) {
					self.DrawMode = 0;
				}
			...
			Obj: {
				...
				CHero: NewO({
					...
					Birth(dom, x, y, width, height, r, bomb = false) {
						let self = this;
						bomb === true && (bomb = 75);
						[self.x, self.y, self.width, self.height, self.dom, self.bomb] = [x, y, width, height,
							dom, bomb
						];
					...
					move(self) {
						...
						self.dom.style.top = `${self.y}px`; {
							...
							P.InitTrigger(P, P.id, P.R, P.C, P.AttackedLX = P.pixelLeft + P.beAttackedPointL, P
								.AttackedRX = P.pixelLeft + P.beAttackedPointR);
					...
					paint(self) {
						...
						ctx.strokeStyle = "red";
						ctx.beginPath();
						ctx.arc(Math.round(self.Trigger.x), Math.round(self.Trigger.y), self.Trigger.r, 0, Math
							.PI * 2, true);
						ctx.closePath();
						ctx.fillStyle = "white";
						ctx.fill();
				...
				CBullet: NewO({
					...
					Birth(data = {}) {
						...
						*/
						...
						self.FadeTime = (_data$FadeTime = data.FadeTime) !== null && _data$FadeTime !== void 0 ?
							_data$FadeTime : data.speed > 4 ? 15 : 30;
						self.Trigger = new selF.Obj.CTrigger();
						self.Trigger.Birth(self.x + self.width / 2, self.y + self.height / 2, data.r ? data.r :
							Math.min(self.width - 1, self.height - 1));
						...
						if (selF.hero && data.angle instanceof Array && data.angle[0] == "hero") {
							self.angle = oMiniGames.oStg.getAngle2(self.Trigger.x, self.Trigger.y, selF.hero
								.Trigger.x, selF.hero.Trigger.y) + data.angle[1];
						}
						...
						self.img = !self.FinalDrawMode ? selF.getImg(data.pic) : selF.getImg(data.pic, data
							.width, data.height, data.rgb);
						selF.bullets[self.id] = self;
						self._defSpd = [Math.cos(self.AngleToRad(self.angle)), Math.sin(self.AngleToRad(self
							.angle))];
					...
					update: function() {
						...
						if (self.task.length > self._task_index && self.liveTime >= self.task[self
								._task_index][0]) {
							self.task[self._task_index++][1](self);
						}

						if (self.x > 900 || self.x + self.width < 0 || self.y + self.height < -50 || self
							.y > 650) {
						...
						if (selF.hero) {
							if (!self.grazed && selF.checkTrigger(self.Trigger, selF.hero.GrazeTrigger)) {
								let sun = AppearSun(self.Trigger.x, self.Trigger.y, 1, 0, 30 - Math
									.random() * 10); //生产阳光
					...
					paint: function() {
						...
						if (self.x !== NaN && self.y !== NaN) {
							if (!$User.LowPerformanceMode && self.liveTime < self.FadeTime) {
								oMiniGames.oStg.RotatePaint(self.img, self.x + self.width / 2 * (self
										.FadeTime - self.liveTime) / self.FadeTime, self.y + self
									.height / 2 * (self.FadeTime - self.liveTime) / self.FadeTime, self
									.width / self.FadeTime * self.liveTime, self.height / self
									.FadeTime * self.liveTime, self.picAngle, self.liveTime / self
									.FadeTime);
							} else {
								oMiniGames.oStg.RotatePaint(self.img, self.x, self.y, self.width, self
									.height, self.picAngle);
							}
						}
					}
				})
	...
	oEffects = {
		...
		Particle({
			...
			for (let i = 0; i < objects.length; i++) {
				...
				objects[i].alpha = (_objects$i$alpha = objects[i].alpha) !== null && _objects$i$alpha !== void 0 ?
					_objects$i$alpha : 1;
				objects[i].rotate = (_objects$i$rotate = objects[i].rotate) !== null && _objects$i$rotate !== void 0 ?
					_objects$i$rotate : 0;
				objects[i].width = (_objects$i$width = objects[i].width) !== null && _objects$i$width !== void 0 ?
					_objects$i$width : -1;
			}
		...
		*/
		ImgSpriter({
			...
			let fun = () => {
				if (index <= frameNum) {
					data ? cssDeclaration.backgroundPosition = data[index - 1] : (cssDeclaration[property] =
						`${pos}px`, pos += changeValue);
					index++;
					return api(fun);
				}
		...
		TextEffects: {
			ZombossEffect(dom, callback) {
				...
				if (!dom) {
					let eff = NewEle("ZombossEffect", "img",
						"position:absolute;z-index:1301;left:166px;top:50px;pointer-events:none;", {
							src: "images/Props/PolarPlots/ZombossTalk0.webp"
						}, EDAll);
					setTimeout(function() {
						eff.src = "images/Props/PolarPlots/ZombossTalk1.webp";
						dom = NewEle("", "div",
							"position: absolute; z-index: 1300; left: 0px; top: 205px; height: 115px; width: 900px; text-align: center; line-height: 115px; color: white; background: rgba(0, 0, 0, 0.6);font-size:24px;", {},
							EDAll);
						callback(dom);
					}, 2533.3);
				} else if ($("ZombossEffect") && !((_$$dataset = $("ZombossEffect").dataset) !== null && _$$dataset !==
						void 0 && _$$dataset.dying)) {
			...
			ShakeText(json) {
				...
				for (let index = 0; index < t.length; index++) {
					...
					if (left >= width - size || delta >= 0) {
						...
						for (let j = 0; j < characters.length; j++) {
							let str = generate(relative, characters[j].character, characters[j].left + delta +
								paddingLeft, characters[j].size, top + paddingTop, font_size, font_family,
								time_range);
							arr.push(str);
						}
...

				{
					...
					for (let j = 0; j < characters.length; j++) {
						let str = generate(relative, characters[j].character, characters[j].left + delta + paddingLeft,
							characters[j].size, top + paddingTop, font_size, font_family, time_range);
						arr.push(
							str
						); //arr.push(`<span style="position:${relative?"relative":"absolute"};left:${characters[j].left+delta+paddingLeft}px;width:${characters[j].size}px;top:${top+paddingTop}px;font-size:${font_size}px;font-family:${font_family};animation: shake_text_1px ${Math.random()*time_range+time_range/2}s linear ${Math.random()-1}s infinite;">${characters[j].character}</span>`);
					}
				};

	...
	window.DataManager = {
		...
		CheckAchievement(type, ...arr) {
			...
			let ps = function(name, ach, type = $__language_Array__["95d001bcead770c985b0a185c2599b35"]) {
				PlaySubtitle(
					`${$__language_Array__["02717ef3f8b4f0a331beec2c48bb8959"][0]}${type}${$__language_Array__["02717ef3f8b4f0a331beec2c48bb8959"][1]}${name}`
				);
        ...
        `;
			...
			let no_border_button_mouseon =
				`onmouseleave="this.style.color='white';" onmouseenter="this.style.color='yellow';"`;
			let backButtonHTML =
				`<a id="close_the_login_pedal" ${no_border_button_mouseon} style="position:absolute;bottom:20px;left:50%;transform:translateX(-50%);${buttonCssText}border:0;font-size:1.2em;">${$__language_Array__["b68f15fdf122a64af6666151910a5f4c"]}</a>`;
			let loginInputBoxStyle =
				`height:1.4em;border-radius:5px;padding-left:10px;background:rgba(0,0,0,0.2);border:1px solid white;color:white;`; //let switchLanguageHTML = `<select id="languageSelect" style="width: 150px;border-radius:3px;"><option value="zh-cn">${"简体中文"}</option><option value="en-us">English</option></select><br/><br/>`;
        ...
        `, {
			...
			$("copy_content").onclick = function() {
				...
				let tmp = NewEle("", "textarea",
					"user-select:all;position:relative;top:0px;left:0px;height:40px;width:300px;margin:0px;", {
						value: saveData,
			...
			$("paste_content").onclick = function showPaste() {
				...
				let dom = self.nextElementSibling;
				let text = NewEle("", "textarea",
					"position:relative;top:0px;left:0px;height:40px;width:300px;margin:0px;", {
				...
				setActiveCopy(text);
				let tmp = NewEle("", "center",
					"position:relative;top:0;left:0px;height:30px;width:300px;color:white;cursor:pointer;", {
						onmouseenter() {
...
(() => {
	oButtons.SetStaticTexts();
	LoadModal('index', 'Service');
})();
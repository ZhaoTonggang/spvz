...

{
	...
	oS.Init({
		PName: [oPeashooter, oWallNut, oPotatoMine, oStoneFlower, oRadish, oSnowPea, oRepeater, oCherryBomb,
			oTallNut, oSunShroom, oPuffShroom, oScaredyShroom, oFumeShroom, oSporeShroom, oBambooUncle,
			oDoomShroom, oBegonia, oPepper, oIceAloe, oImitater, oMonotropa, oSpikeweed, oTorchwood,
			oMiracleImitater, oJalapeno, oLSP
		],
		ZName: [oImp, oZombie, oSadakoZombie, oSkatingZombie, oMakeRifterZombie, oStrollZombie, oFootballZombie,
			oConeheadZombie, oBucketheadZombie, oCigarZombie, oZomboni
		],
		...
		StartGame: function() {
			let [arrP, createTime, moveTime, priorityP] = [oS.PName, 400, 2, [oBambooUncle, oFumeShroom,
				oSporeShroom, oFumeShroom, oBambooUncle, oPuffShroom, oPuffShroom, oSpikeweed,
				oSpikeweed, oSpikeweed, oCherryBomb, oBegonia, oBegonia, oSporeShroom, oJalapeno,
				oCherryBomb, oFumeShroom, oSporeShroom, oFumeShroom, oFumeShroom, oLSP, oDoomShroom,
				oPotatoMine, oTallNut, oRadish, oRadish, oCherryBomb, oRadish, oRadish, oSporeShroom,
				oRadish, oBambooUncle, oBegonia, oBegonia, oTallNut, oTallNut, oPuffShroom, oPuffShroom,
				oPuffShroom, oPuffShroom, oPuffShroom, oPuffShroom, oPuffShroom
			]];
			/* 一些初始化 */

			NewImg("imgKK", "images/interface/ConveyorBelt.webp", "left:0px;top:0px;z-index:0; ", $(
				'dCardList'));
			StopMusic();
			...
			let objFun = {
				...
				ChosePlant(evt, index) {
					...
					let AC = ArCard[oS.ChoseCard],
						...
						evtY = evt.clientY - EDAlloffsetTop + EBody.scrollTop || EElement.scrollTop,
						pro = AC.PName.prototype;
					oS.Chose = 1;
					NewImg("MovePlant", pro.PicArr[pro.StaticGif],
						`left:${evtX - 0.5 * (pro.beAttackedPointL + pro.beAttackedPointR)}px;top:${evtY + 20 - pro.height}px;z-index:258;`,
						FightingScene);
				...
				GrowPlant(l, c, b, f, a) {
					...
					k.CanGrow(l, f, a) && function() {
						PlayAudio(h != 2 ? "plant" + Math.floor(1 + Math.random() * 2) :
							"plant_water");
			...
			PrepareGrowPlants(function() {
				oP.Monitor({
					f: function() {
						...
						const _priorityP = priorityP.map(o => o === 'random' ? arrP
							.random() : o); //生成植物卡牌
						...
						(function createNewCard() {
							...
							if (len < 10) {
								...
								const obj = _priorityP[index] ? _priorityP[index] :
									arrP.random(),
									proto = obj.prototype,
						...
						(function moveCard() {
							...
							while (len--) {
								...
								card.PixelTop > 60 * len //卡牌没有到达顶端
									...
									&&
									($(card.DID).style.top = (card.PixelTop -= 1) +
										"px");
							}
							...
							oSym.addTask(i * 30, _ => {
								...
								oSym.addTask(50, function() {
									if (i == 1) {
										...
										if (j == 5) {
											goLine(zom, 2);
											gotoLine = Math.floor(Math.random() *
												3) + 1;
										} else {
											goLine(zom, 3);
											gotoLine = Math.floor(Math.random() *
					...
					oSym.addTask(700, function ch(times = 0) {
						if (times++ < 15) {
							for (let i = 0; i < zoms.length; i++) {
								...
								zoms[i].HP += 5;
								zoms[i].Speed += 0.05;
								goLine(zoms[i], zoms[i].R <= 1 ? 3 : zoms[i].R >= 5 ? 2 : [
									4, 1, 2, 3, 4
								].random());
							}
						...
						oSym.addTask(500, function ch(times = 0) {
							if (times++ < 30) {
								for (let i = 0; i < zoms.length; i++) {
									...
									zoms[i].HP += 5;
									zoms[i].Speed += 0.05;
									goLine(zoms[i], zoms[i].R <= 1 ? 3 : zoms[i]
										.R >= 5 ? 2 : [1, 1, 2, 3, 4].random());
								}
				...
				for (let i = 1; i < 6; i++) {
					for (let j = 3; j < 10; j++) {
						oSym.addTask(SummonT += 10, _ => {
							let z = PlaceZombie([oBucketheadZombie, oCigarZombie,
								oCigarZombie
							].random(), i, j);
							z.Speed += 0.4;
			...
			oSym.addTask(14000, function() {
				...
				for (let i = 1; i < 6; i++) {
					for (let j = 3; j < 5; j++) {
						oSym.addTask(SummonT += 10, _ => {
							let z = PlaceZombie([oBucketheadZombie, oCigarZombie].random(),
								i, j);
							z.Speed += 0.4;
							oSym.addTask(Math.floor(Math.random() * 50) + 50, function() {
								goLine(z, [1, 4].random());
